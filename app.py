...
# ==========================================
# FIX: st.set_page_config must be the very first Streamlit command to prevent ghost text/warnings
st.set_page_config(
...
# FIX: Added back the missing file path constants to resolve NameError
...
def get_attempt_data(user, test_file):
    ...
    if test_file not in data[user]:
        data[user][test_file] = {'allowed': 5, 'used': 0}
    return data[user][test_file]

def increment_attempt(user, test_file):
    data = load_attempts_data()
    if user not in data: data[user] = {}
    if test_file not in data[user]: data[user][test_file] = {'allowed': 5, 'used': 0}
...
def render_login():
    ...
    with col2:
        with st.container(border=True):
            ...
            # Sort users alphabetically but keep Admin at the top
...
def render_admin():
    ...
    with st.expander("📁 Question Bank Management", expanded=False):
        ...
        for folder in folders:
            with st.container(border=True):
                edit_key = f"edit_folder_{current_admin_path}_{folder}"
                ...
                if is_editing:
                    ...
                    if fc2.button("Rename", key=f"rn_btn_{current_admin_path}_{folder}", use_container_width=True):
                        st.session_state.editing_item = edit_key
                        st.rerun()
                    if fc3.button("Delete", key=f"del_f_{current_admin_path}_{folder}", use_container_width=True):
        ...
        for f_name in files:
            ...
            with st.container(border=True):
                edit_key = f"edit_file_{current_admin_path}_{f_name}"
                is_editing = st.session_state.get('editing_item') == edit_key
                ...
                if is_editing:
                    ...
                    if ec2.button("Save", key=f"save_{edit_key}", type="primary", use_container_width=True):
                        if new_fn and new_fn.strip() != f_name:
                            ...
                            try: os.rename(file_p, os.path.join(full_admin_path, cn))
                            except: pass
                        st.session_state.editing_item = None
                        st.rerun()
                    if ec3.button("Cancel", key=f"cancel_{edit_key}", use_container_width=True):
                    ...
                    tgt = mc1.selectbox("Move", ["-- Move to Folder --"] + all_folders, key=f"mov_{current_admin_path}_{f_name}", label_visibility="collapsed")
                    ...
                    if mc2.button("Rename", key=f"rn_btn_{current_admin_path}_{f_name}", use_container_width=True):
                        st.session_state.editing_item = edit_key
                        st.rerun()
                    
                    if mc3.button("Delete", key=f"del_{current_admin_path}_{f_name}", use_container_width=True):
                        os.remove(file_p)
                        st.rerun()
...
def render_instructions():
    st.markdown("""
    <section class="exam-motivation-banner" aria-label="Exam encouragement">
        <span class="exam-banner-emoji exam-banner-emoji-left" aria-hidden="true">✨</span>
        <div class="exam-motivation-content">
            <h2>✨📚 Best of Luck for Your Exam! 🍀🎯</h2>
            <div class="exam-motivation-lines">
                <span>Believe in Yourself.</span>
                <span>Stay Calm.</span>
                <span>Read Every Question Carefully.</span>
                <span>Manage Your Time Wisely.</span>
            </div>
            <p class="exam-motivation-note">Success comes to those who prepare with dedication.</p>
            <p class="exam-motivation-closing">We wish you all the very best for your examination!</p>
            <p class="exam-motivation-footer" aria-hidden="true">🌟📖💯🚀</p>
        </div>
        <span class="exam-banner-emoji exam-banner-emoji-right" aria-hidden="true">🍀</span>
    </section>
    """, unsafe_allow_html=True)
    ...
    with col2:
        with st.container(border=True):
            ...
            allowed = att_data.get('allowed', 5)
            curr_att = used + 1
            if allowed >= 99:
